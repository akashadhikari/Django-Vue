  ...
  export default {
    ...
    watch: {
      data: function (newVal) {
        this.data = newVal
        this.form = [
          { name: 'service', type: 'select', label: 'Service', model: this.data.service, items: ['TJP', 'HJP', 'FJP', 'GJP'] },
          { name: 'unit', type: 'text', label: 'Unit', model: this.data.unit },
          { name: 'status', type: 'select', label: 'Status', model: this.data.status, items: ['APPROVED', 'PENDING'] },
          // { name: 'type', type: 'select', label: 'Type', model: this.data.type, items: ['Item1', 'Item2'] },
    ...
    methods: {
      ...
      submit () {
        axios({
          method: 'PUT',
          url: 'http://localhost:8000/api/lead/items/' + this.data.id + '/',
